...
public static class GomokuGame
{
    ...
    public struct GameData
    {
        ...
        public PieceType GetPiece(int row, int col)
        {
            ...
            return Board[row * GomokuGame.BOARD_SIZE + col];
        }
...
public class GameOverMessage
{
    ...
    public List<GomokuGame.Position> WinningLine { get; set; } = new List<GomokuGame.Position>();
...
{
    private static GomokuServerManager? _instance;
    private static readonly object _lock = new();
    private GomokuGame.GameData _gameData;
    private readonly Dictionary<int, Player> _players = new();
    private readonly HashSet<int> _playersReady = new(); // 准备开始的玩家列表
    ...
    public static GomokuServerManager Instance
    {
        get
        {
            if (_instance == null)
            {
                lock (_lock)
                {
                    _instance ??= new GomokuServerManager();
                }
            }
            return _instance;
        }
    ...
    private static async Task<bool> OnPlayerDisconnected(object sender, EventPlayerUserDisconnected eventArgs)
    {
        ...
        {
            var player = eventArgs.Player;
            Instance._players.Remove(player.Id);
            Instance._playersReady.Remove(player.Id); // 清除准备状态
    ...
    private PieceType GetPlayerPiece(int playerId)
    {
        ...
        if (playerId == _gameData.Player2Id)
    ...
    private bool CheckWin(int row, int col, PieceType piece)
    {
        ...
        foreach (var direction in directions)
        {
            ...
            // 正方向检查
            for (int i = 1; i < GomokuGame.WIN_CONDITION; i++)
            {
    ...
    private async Task SendGomokuMessage<T>(GomokuMessageType messageType, T data, Player? specificPlayer = null)
    {
        try
        {
            ...
            {
                customMessage.SendTo(specificPlayer);
            }
            else
            {
