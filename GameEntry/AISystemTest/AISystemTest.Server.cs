...
internal class AISystemTestServer : IGameClass
{
    ...
    // AI战斗测试相关
    private static List<Unit> aiCombatUnits = new();
    ...
    private static async Task CreateAICombatUnits()
    {
        ...
        int unitCount = random.Next(6, 12); // 6-12个单位
    ...
    private static async Task AnalyzeCombatSetup()
    {
        ...
        // 分析单位间距离
        for (int i = 0; i < Math.Min(aiCombatUnits.Count, 5); i++)
        {
            var unit1 = aiCombatUnits[i];
            for (int j = i + 1; j < Math.Min(aiCombatUnits.Count, 5); j++)
            {
    ...
    {
        for (int cycle = 0; cycle < 10; cycle++) // 监控10个周期
        {
    ...
    private static async Task WaitForPhaseProgression(string currentPhaseName, string nextPhaseName)
    {
        ...
        await Game.Delay(TimeSpan.FromSeconds(extendedObservationTime));
    ...
    private static async Task TestGuardBehavior()
    {
        ...
        await Game.Delay(TimeSpan.FromSeconds(8));
        ...
        await Game.Delay(TimeSpan.FromSeconds(7));
    }
    ...
    private static async Task TestPatrolBehavior()
    {
        ...
        // 观察完整的巡逻周期
        await Game.Delay(TimeSpan.FromSeconds(15));
    ...
    private static async Task TestFormationBehavior()
    {
        ...
        if (formationWaveAI == null) return;
        ...
        foreach (var (target, index) in formationTargets.Select((t, i) => (t, i)))
        {
            ...
            await Game.Delay(TimeSpan.FromSeconds(6));
        }
    ...
    private static async Task TestCommandSystem()
    {
        ...
        foreach (var (target, index) in testTargets.Select((t, i) => (t, i)))
        {
            ...
            if (result.IsSuccess)
            {
                await Game.Delay(TimeSpan.FromSeconds(4));
            }
        }
    ...
    public static int GetCurrentTestPhase() => currentTestPhase;
    public static int GetTestUnitCount() => testUnits.Count + (leaderUnit != null ? 1 : 0);
    public static string GetWaveAIStatus()
    {
        if (currentWaveAI == null) return "Not initialized";
    ...
    public static bool CanStartTest() => currentTestPhase == 0 && testUnits.Count > 0;
    public static bool IsWaitingForNextPhase() => waitingForNextPhase;
    public static bool IsTestSequenceRunning() => isTestSequenceRunning;
