...
internal class PrimitiveShapeTestServer : IGameClass
{
    ...
    /// 创建单个形状测试
    /// </summary>
    public static async Task CreateSingleShapeTest(PrimitiveShape shape, Vector3 position, Vector3 scale, ShapeColorTheme theme)
    {
        try
        {
    ...
    /// 创建复合形状测试
    /// </summary>
    public static async Task CreateCompositeShapeTest(Vector3 position)
    {
        try
        {
            lastOperationTime = DateTime.Now;
    ...
    /// 清除所有测试单位
    /// </summary>
    public static async Task ClearAllTestUnits()
    {
        try
        {
            lastOperationTime = DateTime.Now;
    ...
    private static async Task CreateTowerUnit(Vector3 position)
    {
        var testPlayer = Player.GetById(2) ?? throw new InvalidOperationException("Test player not found");
        var unit = ScopeData.Unit.HostTestHero.Data?.CreateUnit(
            testPlayer, 
        ...
        if (unit != null)
        {
            testUnits.Add(unit);
            totalUnitsCreated++;
            Game.Logger.LogInformation("🏰 Created tower unit at {position}", position);
        }
    }

    private static async Task CreateSpaceshipUnit(Vector3 position)
    {
        var testPlayer = Player.GetById(2) ?? throw new InvalidOperationException("Test player not found");
        var unit = ScopeData.Unit.HostTestHero.Data?.CreateUnit(
            testPlayer, 
